...
class RobotStatusGUI(QWidget):
    ...
    def update_status(self, status):
        if status == "waiting":
            ...
            self.status_label.setText("목표 도착: 수령 완료 버튼을 눌러주세요.")
            ...
            self.status_label.setText("목표 도착: 수거 완료 버튼을 눌러주세요.")
            self.set_background_image("/home/namsang/serving_robot/src/final_system/resource/image/11.png")
...
class RobotStatusNode(Node, QThread):
    ...
    def status_callback(self, msg):
        try:
            ...
            if command == "deliver":
                ...
                waypoints.append(self.table_coordinates[0])
                ...
                waypoints = [self.table_coordinates[table] for table in table_list if table in self.table_coordinates]
                waypoints.append(self.table_coordinates[10])
                waypoints.append(self.table_coordinates[0])
    ...
    def result_callback(self, future):
        ...
        if status == GoalStatus.STATUS_SUCCEEDED:
            ...
            if self.goal_index < len(self.pose_sequence) - 1:
                ...
                self.status_signal.emit("waiting")
                self.current_command = "waiting"  # Set command to "waiting" after reaching the final goal
            self.goal_index += 1
    ...
    def handle_robot_status_service_request(self, request, response):
        if self.emergency_stopped:
            ...
            response.success = True
            response.message = "2"
        elif self.goal_index == 0 and not self.pose_sequence:
            response.success = True
            response.message = "2"
        elif self.goal_index == len(self.pose_sequence):
            response.success = True
            response.message = "2"
        else:
