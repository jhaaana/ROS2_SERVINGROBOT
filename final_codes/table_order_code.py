...
class Ui_MainWindow(QMainWindow):
    ...
    def setupUi(self):
        ...
        self.top_label = QLabel(f"<독서실>  테이블 번호: {self.ros_node.table_id}", self.top_frame)
        self.top_label.setStyleSheet("background-color:lightgray; border: None;")
        self.top_label.setFont(QFont("Arial", 18, QFont.Bold))
        self.top_label.setAlignment(Qt.AlignCenter)

        self.back_button = QPushButton('<-', self.top_frame)
        self.back_button.setFixedSize(30, 30)
        self.back_button.setStyleSheet("background-color: lightgray; border: 2px solid black;")
